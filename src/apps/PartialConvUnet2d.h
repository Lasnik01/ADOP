...
class GatedBlockImpl : public UnetBlockImpl
{
   public:
                   ...
                   std::string norm_str = "bn", std::string activation_str = "elu")
    {
        ...
        feature_conv = torch::nn::Conv2d(torch::nn::Conv2dOptions(in_channels, out_channels, kernel_size)
                                             .stride(stride)
                                             .dilation(dilation)
                                             .padding(n_pad_pxl));
        feature_transform->push_back(feature_conv);
        feature_transform->push_back(ActivationFromString(activation_str));
        mask_conv = torch::nn::Conv2d(torch::nn::Conv2dOptions(in_channels, out_channels, kernel_size)
                                          .stride(stride)
                                          .dilation(dilation)
                                          .padding(n_pad_pxl));
        ...
        if(in_channels == 4 && out_channels == 16)
        {
            myConv = MyConvUnet2d(in_channels, out_channels, activation_str);
            useMyConv = true;
        }
    }
    ...
    bool useMyConv = false;
    MyConvUnet2d myConv;
    torch::nn::Conv2d feature_conv = nullptr;
    torch::nn::Conv2d mask_conv = nullptr;
